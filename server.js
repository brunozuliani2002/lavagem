...
app.post("/carros", (req, res) => {
    const { nome, cor, placa, valor } = req.body;
    const data = new Date().toISOString().split("T")[0]; // Data atual no formato YYYY-MM-DD
    db.run("INSERT INTO carros (nome, cor, placa, valor, data) VALUES (?, ?, ?, ?, ?)",
        [nome, cor, placa, valor, data],
        function (err) {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
            res.json({ id: this.lastID, nome, cor, placa, valor, data });
        }
    );
});
